...
template <uint32_t blockSize>
__global__ void reduce_gpu_v6_kernel(const uint32_t *g_data, const int warp_size, uint32_t*g_out){
...
uint32_t reduce_gpu_v6(const uint32_t *d_data, uint32_t *h_tmp_out, uint32_t *d_tmp_out, size_t n, int n_blocks, int n_threads, cudaDeviceProp &prop) {
    size_t n_blocks_v6 = n_blocks/2;
    assert(512==n_threads);
    reduce_gpu_v6_kernel<512><<<n_blocks_v6, n_threads, sizeof(uint32_t)*n_threads>>>(d_data, prop.warpSize, d_tmp_out);
    checkCudaErrors(cudaMemcpy(h_tmp_out, d_tmp_out, sizeof(uint32_t)*n_blocks_v6, cudaMemcpyDefault));
    uint32_t ret = reduce(h_tmp_out, n_blocks_v6);
    return ret;
}
*/
