...
int reduce_gpu_v4(const int *d_in, int *h_tmp_out, int *d_tmp_out, size_t n, int n_blocks, int n_threads)
{
    ...
    reduce_gpu_v4_kernel<<<n_blocks_v4, n_threads, sizeof(int) * n_threads>>>(d_in, n, d_tmp_out);
...
int reduce_gpu_v6(const int *d_in, int *h_tmp_out, int *d_tmp_out, size_t n, int n_blocks, int n_threads)
{
    ...
    int n_blocks_v4 = n_blocks / 2;
    reduce_gpu_v6_kernel<512><<<n_blocks_v4, n_threads, sizeof(int) * n_threads>>>(d_in, n, d_tmp_out);
    checkCudaErrors(cudaMemcpy(h_tmp_out, d_tmp_out, sizeof(int) * n_blocks_v4, cudaMemcpyDefault));
    int ret = reduce(h_tmp_out, n_blocks_v4);
    return ret;
}
